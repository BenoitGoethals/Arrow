...
def _build_platoon() -> tuple[list[SimOp], list[SimSection]]:
    ...
    for sec_num, (sec_name, offsets) in enumerate(
        [("1 SEC", _1SEC_OFFSETS), ("2 SEC", _2SEC_OFFSETS), ("3 SEC", _3SEC_OFFSETS)],
        start=1,
    ):
        ...
        for i, (fwd, rgt) in enumerate(offsets):
            ...
            rank = (
                "OR-6"
                if (team_num == 1 and mbr_num == 1)
                else "OR-5" if mbr_num == 1 else "OR-3"
            )
            op = SimOp(
                callsign=callsign, rank=rank, role="OPERATOR", fwd_m=fwd, right_m=rgt
            )
...
async def bootstrap(
    ...
    if ARGS.reset:
        ...
        for row in existing:
            if row.get("callsign") in sim_cs:
                await _api(
                    client, "DELETE", f"/operators/{row['id']}", token=admin_token
                )
    ...
    platoon = next(
        (
            ...
            if p["name"] == "1 PLT" and p["company_id"] == company_id
        ),
        None,
    )
    if not platoon:
    ...
    sec_name_to_id = {
        s["name"]: s["id"] for s in sections_resp if s["platoon_id"] == platoon_id
    }
    ...
    existing_ops = {
        o["callsign"]: o
        for o in (await _api(client, "GET", "/operators", token=admin_token) or [])
    }
    for sec in sections:
        n_teams = 1 if sec.name == "HQ SEC" else 3
        ...
        for i, op in enumerate(sec.operators):
            ...
            if not ex:
                ...
                if r:
                    ...
                    log.info(
                        "  Registered %-14s (%s) -> %s", op.callsign, op.rank, team_name
                    )
            else:
                op.op_id = ex["id"]
...
async def login_all(client: httpx.AsyncClient, all_ops: list[SimOp]) -> None:
    ...
    log.info(
        "-- Tokens: %d from registration, %d need login --", already, len(need_login)
    )
    ...
    log.info(
        "  Tokens ready: %d from reg + %d from login  (%d failed)", already, ok, fail
    )
...
async def plant_opord(client: httpx.AsyncClient, admin_token: str) -> None:
    ...
    if any(isinstance(o, dict) and o.get("opord_number") == opord_no for o in existing):
    ...
    await _api(
        ...
        token=admin_token,
        json={
            "label": "1 PLT AO -- Dendermonde to Aalst",
            "bbox": [50.920, 4.020, 51.045, 4.120],
            "zoom": 13,
...
async def plant_enemy(client: httpx.AsyncClient, admin_token: str) -> list[OpforUnit]:
    ...
    while True:
        for u in units:
            ...
            new_lat, new_lon = offset_point(
                u.lat, u.lon, step_m * math.cos(ang), step_m * math.sin(ang)
            )
            if dist_m(u.orig_lat, u.orig_lon, new_lat, new_lon) > u.jitter_m:
                continue
            await _api(
                client, "DELETE", f"/tactical-objects/{u.obj_id}", token=admin_token
            )
            r = await _api(
                client,
                "POST",
...
async def run_drone_spots(
    ...
    while True:
        ...
        payload = {
            ...
            "distance": int(dist_m(drone_op.lat, drone_op.lon, target.lat, target.lon)),
            "description": notes,
        }
        r = await _api(
            client,
            "POST",
            "/reports",
            token=drone_op.token,
            json={"type": "SPOT", "payload": payload},
        )
        if r:
            log.info(
                "SHADOW-1 SPOT #%-3d  %-12s  %.0f m away",
                count,
                target.etype,
                payload["distance"],
            )
        await _api(
            ...
            token=drone_op.token,
            json={
                "content": f"SHADOW-1: {notes} -- grid {payload['grid']}",
    ...
    while True:
        ...
        for u in units:
            if u.tic_fired:
                ...
                if fwd_ops and all(
                    dist_m(o.lat, o.lon, u.lat, u.lon) > TIC_RANGE_M * 3
                    for o in fwd_ops
                ):
                    u.tic_fired = False
                continue
            ...
            if random.random() < 0.4:
                ...
                plt_sgt = next(
                    (o for o in all_ops if o.callsign == "1PLT-HQ2" and o.token),
                    closest,
                )
                r = await _api(
                    client,
                ...
                if r:
                    log.info(
                        "CASEVAC #%d  %s  %s", cas_count, closest.callsign, injury[:45]
                    )
                await _api(
                    client,
                    "POST",
...
async def run_platoon(
    client: httpx.AsyncClient, all_ops: list[SimOp], speed_mult: float
) -> None:
    real_dt = UPDATE_S / speed_mult
    centre_lat, centre_lon = WAYPOINTS[0]
    wp_idx = 1
    bearing = march_bearing(
        centre_lat, centre_lon, WAYPOINTS[wp_idx][0], WAYPOINTS[wp_idx][1]
    )
    for op in all_ops:
        op.lat, op.lon = formation_pos(
            centre_lat, centre_lon, op.fwd_m, op.right_m, bearing
        )
...
async def main() -> None:
    ...
    log.info(
        "Backend: %s  |  Operators: %d  |  Speed: %.1fx",
        _CTX.base,
        len(all_ops),
        ARGS.speed,
    )
    log.info(
        "Walk %.1f km/h  update=%.0fs  drone=%.0fs  TIC=%.0fs",
        WALK_MS * 3.6,
        UPDATE_S / ARGS.speed,
        DRONE_S / ARGS.speed,
    ...
    async with httpx.AsyncClient(verify=False) as client:
        ...
        _CTX.mission_id = await sim_utils.create_mission_async(
            ...
            map_center_lat=mid_lat,
            map_center_lng=mid_lon,
            map_zoom=13,
        )
        if _CTX.mission_id:
