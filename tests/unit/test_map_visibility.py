...
def test_map_and_notif_are_independent(client) -> None:
    ...
    r = client.put(
        "/admin/map-visibility", headers=auth(admin_tok), json={"notif_alerts": False}
    )
    body = r.json()
...
def test_chat_toast_toggle_persists(client) -> None:
    _, admin_tok, _ = register(client, "ADMIN")
    r = client.put(
        "/admin/map-visibility", headers=auth(admin_tok), json={"notif_chat": False}
    )
...
def test_partial_patch_keeps_other_fields(client) -> None:
    ...
    r = client.put(
        "/admin/map-visibility", headers=auth(admin_tok), json={"alerts": False}
    )
    body = r.json()
...
def test_any_operator_can_toggle_defaults(client) -> None:
    ...
    for tok in (op_tok, bc_tok):
        r = client.put(
            "/admin/map-visibility", headers=auth(tok), json={"alerts": False}
        )
        assert r.status_code == 200, r.text
        assert r.json()["alerts"] is False
        # Reset for the next iteration.
        client.put("/admin/map-visibility", headers=auth(tok), json={"alerts": True})
...
def test_persists_across_reset(client) -> None:
    ...
    _, admin_tok, _ = register(client, "ADMIN")
    client.put(
        "/admin/map-visibility", headers=auth(admin_tok), json={"cot_tracks": False}
    )
