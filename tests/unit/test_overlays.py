...
def test_create_list_get(client: TestClient) -> None:
    ...
    r = client.post("/overlays", headers=auth(tok), json={
        "name": "Op Alpha",
...
def test_unknown_object_ids_are_dropped(client: TestClient) -> None:
    ...
    r = client.post("/overlays", headers=auth(tok), json={
        "name": "Mixed", "object_ids": [real, 99999, 88888],
    })
...
def test_patch_round_trip(client: TestClient) -> None:
    ...
    overlay = client.post("/overlays", headers=auth(tok), json={
        "name": "Draft", "object_ids": ids[:1],
    }).json()

    r = client.patch(f"/overlays/{overlay['id']}", headers=auth(tok), json={
        "name": "Final",
        "object_ids": ids,
    })
...
def test_operator_cannot_mutate(client: TestClient) -> None:
    ...
    r = client.post("/overlays", headers=auth(op_token), json={
        "name": "Forbidden", "object_ids": ids,
    })
    assert r.status_code == 403

    # Or delete.
    overlay = client.post("/overlays", headers=auth(admin_tok), json={
        "name": "X", "object_ids": ids,
    }).json()
    assert client.delete(f"/overlays/{overlay['id']}", headers=auth(op_token)).status_code == 403
...
def test_delete(client: TestClient) -> None:
    ...
    overlay = client.post("/overlays", headers=auth(tok), json={
        "name": "Doomed", "object_ids": ids,
    }).json()
    assert client.delete(f"/overlays/{overlay['id']}", headers=auth(tok)).status_code == 204
    assert client.get(f"/overlays/{overlay['id']}", headers=auth(tok)).status_code == 404
...
def test_empty_object_list_is_valid(client: TestClient) -> None:
    ...
    tok = admin_token(client)
    r = client.post("/overlays", headers=auth(tok), json={
        "name": "Empty placeholder", "object_ids": [],
    })
    assert r.status_code == 201
