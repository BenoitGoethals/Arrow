...
AGENTS = {
    ...
    "B": [
        (
            "Anthrax (B. anthracis)",
            "MISSILE",
            "Spore-forming — persistent soil contamination",
        ),
...
def build_scenario(rng: random.Random) -> list[Strike]:
    ...
    def add(
        target_idx: int, cbrn_type: str, minutes_ago: int, force_cat: str | None = None
    ) -> None:
        ...
        delivery = rng.choices([pref_delivery, rng.choice(DELIVERIES)], weights=[4, 1])[
            0
        ]
    ...
    east_arctic = [
        i for i, t in enumerate(TARGETS) if t[3] in ("Far East", "Arctic", "E. Siberia")
    ]
    ...
    cities = [i for i, t in enumerate(TARGETS) if t[4] == 1 and t[3] not in ("Arctic",)]
