...
_FIRST = [
    ...
    "Emily","Angela","Rebecca","Katherine","Michelle","Christine",
]
_LAST = [
    ...
    "Fisher","Ellis","Harrison","Gibson","Marshall","Kennedy","Warren","Dixon",
    "Ramos","Burns","Gordon","Shaw","Holmes","Rice","Robertson","Hunt","Black",
    "Daniels","Palmer","Mills","Spencer","Pierce","Hawkins","Grant","Webb",
    "Stone","Cross","Flint","Blackwood","Whitfield","Holt","Walsh","Chen",
]
...
def _op(rank: str, role: str = "OPERATOR", team_role: str = "OPERATOR",
        callsign: str | None = None,
        first: str | None = None, last: str | None = None) -> dict[str, Any]:
...
def _rifle_section(prefix: str) -> dict:
    return {"name": f"{prefix} Section", "teams": [
...
def _plt_hq_section(prefix: str, pl_rank: str = "1LT",
    ...
    return {"name": f"{prefix} PLT HQ Sec", "teams": [
        {"name": f"{prefix} HQ Team", "members": [
...
def _weapons_platoon(coy: str) -> dict:
    return {"name": f"{coy} WPNS PLT", "sections": [
        {"name": f"{coy} MORT Section", "teams": [
...
def _coy_hq_plt(coy: str,
                co_cs: str, co_fn: str, co_ln: str,
                xo_cs: str, xo_fn: str, xo_ln: str,
                sg_cs: str, sg_fn: str, sg_ln: str) -> dict:
    return {"name": f"{coy} HQ PLT", "sections": [
        {"name": f"{coy} HQ Section", "teams": [
...
def _rifle_company(coy_name: str, prefix: str,
                   co_cs: str, co_fn: str, co_ln: str,
                   xo_cs: str, xo_fn: str, xo_ln: str,
                   sg_cs: str, sg_fn: str, sg_ln: str) -> dict:
    return {"name": coy_name, "platoons": [
        _coy_hq_plt(coy_name, co_cs, co_fn, co_ln,
                    xo_cs, xo_fn, xo_ln, sg_cs, sg_fn, sg_ln),
...
def build_battalion() -> list[dict]:
    return [
        # ── HHC ─────────────────────────────────────────────────────────────
        {"name": "HHC 1-75 RGR", "platoons": [
            {"name": "BN CMD PLT", "sections": [
                {"name": "BN CMD Section", "teams": [
                    {"name": "BN HQ", "members": [
            ...
            {"name": "SPT PLT", "sections": [
                {"name": "MORT Section", "teams": [
                ...
                {"name": "MED Section", "teams": [
                    {"name": "Aid Station", "members": [
                        _op("CPT", "BATTLE_CAPTAIN", "PA",
                            callsign="TORRES-PA", first="Jennifer", last="Torres"),
        ...
        # ── Rifle Companies ──────────────────────────────────────────────────
        _rifle_company("ALPHA CO 1-75 RGR", "ALPHA",
                       "COLE",      "Ryan",   "Cole",
                       "HUNT",      "Derek",  "Hunt",
                       "RODRIGUEZ", "James",  "Rodriguez"),
        _rifle_company("BRAVO CO 1-75 RGR", "BRAVO",
                       "PIERCE",    "Marcus", "Pierce",
                       "WEBB",      "Nathan", "Webb",
                       "SIMMONS",   "Kevin",  "Simmons"),
        _rifle_company("CHARLIE CO 1-75 RGR", "CHARLIE",
                       "HAWKINS",   "Eric",   "Hawkins",
                       "GRANT",     "Cody",   "Grant",
                       "MOORE",     "Travis", "Moore"),
        _rifle_company("DELTA CO 1-75 RGR", "DELTA",
                       "SHAW",      "Tyler",  "Shaw",
                       "REED",      "Justin", "Reed",
                       "WALKER",    "Adam",   "Walker"),
    ]
...
class Session:
    ...
    def login(self) -> bool:
        try:
            r = self._client.post(f"{BASE}/auth/login",
                                  data={"username": self._callsign,
    ...
    def post(self, path: str, body: dict,
             mission_id: int | None = None) -> dict | None:
        for attempt in range(2):
            try:
                r = self._client.post(f"{BASE}{path}", headers=self._hdr(mission_id),
    ...
    def patch(self, path: str, body: dict) -> bool:
        for attempt in range(2):
            try:
                r = self._client.patch(f"{BASE}{path}", headers=self._hdr(),
                                       json=body, timeout=15)
    ...
    def delete(self, path: str) -> bool:
        for attempt in range(2):
            try:
                r = self._client.delete(f"{BASE}{path}", headers=self._hdr(), timeout=15)
                if r.status_code in (200, 204):
                    return True
                if r.status_code == 401 and attempt == 0:
                    self.login()
...
BN_COMPANY_NAMES = {
    "HHC 1-75 RGR", "ALPHA CO 1-75 RGR",
    "BRAVO CO 1-75 RGR", "CHARLIE CO 1-75 RGR", "DELTA CO 1-75 RGR",
}
...
def reset(sess: Session, battalion: list[dict]) -> None:
    ...
    bn_callsigns = {m["callsign"] for coy in battalion
...
def bootstrap(sess: Session, battalion: list[dict],
            ...
            for sec in plt["sections"]:
                ...
                for team in sec["teams"]:
                    ...
                    if team_name in existing_teams:
                        ...
                        r = sess.post("/teams", {"name": team_name, "section_id": sec_id})
                        if not r:
                            continue
                        team_id = r["id"]
                    ...
                    for member in team["members"]:
                        ...
                        reg = sess.post("/auth/register/admin", {
                            "callsign": cs,
                            "password": PASSWORD,
                            "rank":     member["rank"],
                            "role":     member["role"],
                        ...
                        if new_token:
                            try:
                                me = sess._client.get(f"{BASE}/auth/me",
    ...
    log.info("Operators: %d created, %d already existed, %d total", created, skipped, total)
    return total
...
def main() -> None:
    ...
    with httpx.Client(verify=False) as client:
        ...
        if not sess.login():
            log.error("Cannot log in as '%s'. Ensure the backend is running and "
                      "seeded (uv run arrow-backend).", ARGS.seed_admin)
            sys.exit(1)
        ...
        mission_id = sim_utils.create_mission_sync(
            ...
            map_center_lat=50.0028, map_center_lng=5.7300, map_zoom=12,
        )
        if mission_id:
