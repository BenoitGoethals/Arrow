...
def login(client: httpx.Client, callsign: str, password: str) -> str:
    r = client.post(
        _p("/auth/login"), data={"username": callsign, "password": password}
    )
    ...
    if payload.get("mfa_required"):
        sys.exit(
            "admin account has MFA enabled — use a non-MFA admin for the simulator"
        )
...
def build_company_plan(
    coy: str, obj_name: str, village: str, centre: LatLon, axis_bearing: float
) -> list[dict]:
    ...
    flet_right = centre.bearing_m(axis_bearing, DEPTH_FLET).bearing_m(
        perp_right, RIGHT_M
    )
    ...
    def line(type_: str, pts: list[LatLon]) -> dict:
        return tg(
            ...
            pts[0].lat,
            pts[0].lon,
            geometry=f'{{"type":"line","coords":{[p.as_pair() for p in pts]}}}'.replace(
                "'", '"'
            ),
        )
    ...
    def poly(type_: str, pts: list[LatLon], **kw) -> dict:
        d = tg(
            ...
            pts[0].lat,
            pts[0].lon,
            geometry=f'{{"type":"polygon","coords":{[p.as_pair() for p in pts]}}}'.replace(
                "'", '"'
            ),
            **kw,
        )
    ...
    obj_poly = [
        centre.offset_m(350, -300),
        centre.offset_m(350, 300),
        centre.offset_m(-250, 350),
        centre.offset_m(-250, -350),
    ]
    items.append(
        poly(
    ...
    poi_units = [
        ...
        (
            "POL point",
            "SFGPIRP-----",
            aa.bearing_m(axis_bearing, 450).bearing_m(perp_right, 200),
        ),
        (
            "HQ / TOC",
            "SFGPUH------",
            aa.bearing_m(axis_bearing, 250).bearing_m(perp_left, 150),
        ),
    ]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Arrow battlefield — Operation IRON ARDENNES"
    )
    parser.add_argument(
        "--backend",
    ...
    parser.add_argument(
        "--mission-name",
        default="Operation Iron Ardennes",
    ...
    log.info(
        "Done. %d / %d objects planted across %d company plans.",
        total_ok,
        total_all,
        len(PLANS),
    )
    log.info(
        "Centre of mass: Belgian Ardennes (~50.15 N, 5.75 E). Open the "
