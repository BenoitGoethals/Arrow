...
parser = argparse.ArgumentParser(
    description="Arrow regiment simulator — OPERATION IRON SKY (Kananga)"
)
parser.add_argument(
    "--backend",
...
parser.add_argument("--once", action="store_true", help="Stop after Φ5 (no loop)")
parser.add_argument(
    "--mission-name",
    default="Operation Iron Sky",
...
PHASES: list[Phase] = [
    Phase(
        0, "AMBER", "DZ NORTH", "Airborne insertion — secure DZ", DZ_NORTH, 180.0
    ),  # paras descend N→S toward airfield
    Phase(
        1,
        "BRONZE",
        "FUP BLADE",
        "DZ secure — form up and advance to airfield perimeter",
        FUP,
        180.0,
    ),
    Phase(2, "CRIMSON", "OBJ EAGLE", "Assault Kananga Airfield", AIRFIELD, 180.0),
    Phase(
        3, "IRON", "OBJ STATION", "C Coy advances to seize train station", STATION, 95.0
    ),  # eastward advance through city
    Phase(
        4,
        "GRANITE",
        "DEF POSTURE",
        "Consolidate — establish all-round defence",
        AIRFIELD,
        0.0,
    ),
]
...
def build_static_objects() -> list[dict]:
    ...
    items.append(
        _line(
            "BOUNDARY",
            [reg_bdy_s2, reg_bdy_n2],
            echelon="REG",
            notes="Right boundary — REG axis",
        )
    )
    ...
    items.append(
        _line(
            ...
            [bdy_ab_n, bdy_ab_s],
            echelon="COY",
            notes="A/B Coy boundary — runway centreline",
        )
    )
    ...
    enemy_units = [
        ...
        (
            "Fence-line fighting position",
            "SHGPUCI-----",
            AIRFIELD_N.bearing_m(270, 150),
        ),
        ("Fence-line fighting position", "SHGPUCI-----", AIRFIELD_N.bearing_m(90, 150)),
        ...
        (
            "PKM team, control-tower roof",
            "SHGPUCIS----",
            AIRFIELD_CTRL.bearing_m(0, 80),
        ),
        ("RPG team, apron hangar", "SHGPUCAA---F", AIRFIELD.bearing_m(270, 300)),
        ...
        (
            "Technical w/ DShK (N approach)",
            "SHGPEVAT----",
            AIRFIELD.bearing_m(0, 700).bearing_m(270, 100),
        ),
        (
            "Technical w/ DShK (apron W)",
            "SHGPEVAT----",
        ...
        ("Militia PKM team, station roof", "SHGPUCIS----", STATION.bearing_m(0, 100)),
        (
...
def _regiment_orbat() -> list[SimOp]:
    return [
        ...
        SimOp(
            "ARROW-S3", "OF-4", "BATTLE_CAPTAIN", offset_n=-300, offset_e=120
        ),  # S3 ops
        SimOp(
            "ARROW-S2", "OF-3", "BATTLE_CAPTAIN", offset_n=-300, offset_e=-120
        ),  # S2 int
        SimOp(
            "ARROW-FAC", "OF-3", "BATTLE_CAPTAIN", offset_n=-150, offset_e=80
        ),  # JTAC/FAC
        # ── 1 Para Bn ──────────────────────────────────────────────────────
        SimOp(
            "LAMBERT-6", "OF-4", "BATTLE_CAPTAIN", offset_n=-100, offset_e=0
        ),  # 1 BN CDR
        SimOp(
            "ACOY-6", "OF-3", "BATTLE_CAPTAIN", offset_n=200, offset_e=-200
        ),  # A Coy OC
        SimOp(
            "BCOY-6", "OF-3", "BATTLE_CAPTAIN", offset_n=200, offset_e=200
        ),  # B Coy OC
        SimOp(
            "CCOY-6", "OF-3", "BATTLE_CAPTAIN", offset_n=-400, offset_e=0
        ),  # C Coy OC (reserve)
        SimOp(
            "ALPHA-11", "OR-6", "OPERATOR", offset_n=450, offset_e=-350
        ),  # A Coy 1 Pl
        SimOp(
            "ALPHA-21", "OR-6", "OPERATOR", offset_n=400, offset_e=-150
        ),  # A Coy 2 Pl
        ...
        SimOp(
            "CHARLIE-61", "OR-6", "OPERATOR", offset_n=-600, offset_e=-100
        ),  # C Coy 6 Pl
        SimOp(
            "CHARLIE-71", "OR-6", "OPERATOR", offset_n=-700, offset_e=100
        ),  # C Coy 7 Pl
        ...
        SimOp(
            ...
            offset_n=0,
            offset_e=0,
            home=DZ_NORTH.offset_m(200, -300),
        ),
        SimOp(
            "ECHO-91",
            "OR-6",
        ...
        SimOp(
            "THUNDER-6", "OF-3", "BATTLE_CAPTAIN", offset_n=0, offset_e=0, home=GUN_LINE
        ),
        SimOp(
            "THUNDER-FDC",
            "OR-5",
        ...
        SimOp(
            ...
            offset_n=0,
            offset_e=0,
            home=DZ_NORTH.offset_m(0, 500),
        ),
        SimOp("SUPPLY-6", "OR-7", "OPERATOR", offset_n=0, offset_e=0, home=DZ_EAST),
    ]
...
async def drive_operator(
    client: httpx.AsyncClient, op: SimOp, phase_idx_state: dict, speed: float
) -> None:
    if not op.token:
        return
    # Seed near DZ NORTH
    ...
    while True:
        ...
        payload = {
            ...
            "equipment": random.choice(
                ["AK-47", "PKM", "RPG-7", "60mm mortar", "technical"]
            ),
...
async def inject_81mm_fire_missions(
    client: httpx.AsyncClient, ops: list[SimOp], speed: float, phase_idx_state: dict
) -> None:
    ...
    phase_targets = {
        ...
        1: [  # Φ2 BRONZE — registration + SMOKE
            (
                AIRFIELD_N.bearing_m(0, 500).lat,
                AIRFIELD_N.bearing_m(0, 500).lon,
                "ADJUST_FIRE",
                "HE",
                3,
            ...
            (
                AIRFIELD_CTRL.bearing_m(270, 300).lat,
                AIRFIELD_CTRL.bearing_m(270, 300).lon,
                "SUPPRESSION",
                "SMOKE",
                4,
        ...
        2: [  # Φ3 CRIMSON — main assault fire support
            ...
            (
                AIRFIELD.bearing_m(180, 1_200).lat,
                AIRFIELD.bearing_m(180, 1_200).lon,
                "SUPPRESSION",
                "HE",
                4,
        ...
        3: [  # Φ4 IRON — C Coy advance to station
            (
                STATION.bearing_m(270, 500).lat,
                STATION.bearing_m(270, 500).lon,
                "FIRE_FOR_EFFECT",
                "HE",
                6,
            ...
            (
                STATION.bearing_m(270, 600).lat,
                STATION.bearing_m(270, 600).lon,
                "SUPPRESSION",
                "SMOKE",
                4,
            ...
            (
                CITY_CENTRE.bearing_m(90, 400).lat,
                CITY_CENTRE.bearing_m(90, 400).lon,
                "SUPPRESSION",
                "HE",
                3,
        ...
        4: [  # Φ5 GRANITE — consolidation
            (
                AIRFIELD.bearing_m(180, 1_400).lat,
                AIRFIELD.bearing_m(180, 1_400).lon,
                "ILLUMINATION",
                "ILLUM",
                2,
            ...
            (
                STATION.bearing_m(180, 600).lat,
                STATION.bearing_m(180, 600).lon,
                "ILLUMINATION",
                "ILLUM",
                2,
    ...
    while True:
        ...
        await api(
            ...
            json={
                "content": (
                    f"FIRE MISSION — 81mm {ammo} ×{qty} {mtype}\n"
...
async def inject_tic_alerts(
    ...
    while True:
        ...
        type_ = random.choices(
            ["TIC", "MEDICAL", "EVAC", "LOST_COMMS"], weights=[7, 2, 1, 1]
        )[0]
        r = await api(
            client,
            "POST",
...
async def inject_phase_radio(
    client: httpx.AsyncClient, ops: list[SimOp], phase_idx_state: dict, speed: float
) -> None:
    ...
    phase_messages = {
        0: (
            "MOREAU-6",
            "ALL STATIONS — H-HOUR. Serial 1 away. AMBER AMBER AMBER. "
            "DZ NORTH: 1 BN stick away. Wind 080/12. No chutes fouled. OUT.",
        ),
        1: (
            "LAMBERT-6",
            "BLADE this is STEEL — all callsigns at FUP. "
        ...
        2: (
            "ACOY-6",
            "MOREAU-6 this is ACOY — EAGLE ASSAULT BEGUN. "
        ...
        3: (
            "CCOY-6",
            "EAGLE secured. CCOY now on LANCE axis eastward. "
        ...
        4: (
            "MOREAU-6",
            "ALL STATIONS — IRON SKY IRON SKY IRON SKY. "
...
async def amain() -> None:
    ...
    async with httpx.AsyncClient(timeout=20.0, verify=False) as client:
        ...
        _CTX.mission_id = await sim_utils.create_mission_async(
            ...
            admin_token,
            ARGS.mission_name,
            map_center_lat=-5.900,
            map_center_lng=22.368,
            map_zoom=14,
        )
        ...
        if ARGS.reset:
            ...
            log.info(
                "Reset: %d TGs · %d OPORDs · %d sim operators removed.",
                n_obj,
                n_op,
                n_users,
            )
        ...
        log.info(
            "    Kananga Airport  (OBJ EAGLE):   %.4f, %.4f", AIRFIELD.lat, AIRFIELD.lon
        )
        log.info(
            "    Train Station    (OBJ STATION): %.4f, %.4f", STATION.lat, STATION.lon
        )
        log.info(
            "    DZ NORTH:                       %.4f, %.4f", DZ_NORTH.lat, DZ_NORTH.lon
        )
        ...
        tasks = [
            ...
            asyncio.create_task(inject_contacts(client, active, ARGS.speed)),
            asyncio.create_task(inject_spot_reports(client, active, ARGS.speed)),
            asyncio.create_task(
                inject_81mm_fire_missions(client, active, ARGS.speed, phase_state)
            ),
            asyncio.create_task(inject_tic_alerts(client, active, ARGS.speed)),
            asyncio.create_task(
        ...
        for op in active:
            tasks.append(
                asyncio.create_task(drive_operator(client, op, phase_state, ARGS.speed))
            )
