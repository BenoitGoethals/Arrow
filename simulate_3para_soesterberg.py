...
ROSTER: list[OpRoster] = [
    ...
    OpRoster(
        "CHARLIE-6", "OF-2", "BATTLE_CAPTAIN", "CHARLIE", "CHARLIE-HQ", "DZ_GREEN"
    ),
    ...
    OpRoster(
        "RECON-6", "OF-1", "INTEL", "ECHO", "RECON-TM", "DZ_MAROON"
    ),  # jumps with main
...
def build_graphics(mgrs_fn) -> list[dict]:
    ...
    items.append(
        poly(
            "OBJ_AREA",
            [
                terminal.offset(+200, -300),
                terminal.offset(+200, +500),
                terminal.offset(-200, +500),
                terminal.offset(-200, -300),
            ],
            echelon="COY",
            notes="OBJ AMBER — terminal complex + hangars (ALPHA CIE main effort)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="BN",
            notes="OBJ ORANGE — RWY 09/27 + taxiways (BRAVO seizes for CLP)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="BN",
            notes="OBJ NASSAU — Amersfoort Kazerne / Bernhardkazerne compound",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="PL",
            notes="DZ MAROON — BHQ / ALPHA / BRAVO / DELTA (H-Hour, 1st pass)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="PL",
            notes="DZ GREEN — CHARLIE CIE (H-Hour, simultaneous, SE flank)",
        )
    )
    ...
    items.append(
        line(
            "PHASE_LINE",
            [
                kaz_w_gate.offset(0, -600),
                kaz_n_gate.offset(0, +400),
            ],
            echelon="COY",
    ...
    enemies_air = [
        ...
        (
            "OPFOR mortar position — hangar area",
            "SHGPUCF-----",
            hangars.offset(+200, 0),
        ),
    ]
    ...
    enemies_kaz = [
        ...
        (
            "OPFOR BMP-2 — Kazerne N perimeter #1",
            "SHGPEVAT----",
            kaz.offset(+200, -100),
        ),
        (
            "OPFOR BMP-2 — Kazerne N perimeter #2",
            "SHGPEVAT----",
            kaz.offset(+200, +200),
        ),
        ...
        (
            "OPFOR AT bunker — N gate (Fagot/Kornet)",
            "SHGPUCAW----",
            kaz_n_gate.offset(-100, 0),
        ),
        ("OPFOR AT bunker — W gate", "SHGPUCAW----", kaz_w_gate.offset(0, +200)),
        (
            "OPFOR mortar section — barracks courtyard",
            "SHGPUCF-----",
            kaz.offset(-100, +100),
        ),
        ("OPFOR sniper — N water tower", "SHGPUCRV----", kaz.offset(+350, -50)),
        (
            "OPFOR QRF route — N237 northbound",
            "SHGPUCIZ----",
            n237_block.offset(0, +1500),
        ),
    ]
    ...
    pois = [
        ...
        (
            "LZ MAROON — CASEVAC / DUSTOFF ANGEL 31",
            "SFGPIBA-----",
            dz_m.offset(-200, +100),
        ),
        ...
        (
            "PION IED clearance route (pre-CLP)",
            "SFGPIBE-----",
            rwy_09.offset(+50, +300),
        ),
        # Kazerne approach
        ...
        (
            "RECON screen OP — N237/N221 jct",
            "SFGPUH------",
            n237_block.offset(+500, +1000),
        ),
    ...
    items.append(
        poly(
            "OBJ_AREA",
            [
                terminal.offset(+150, -330),
                terminal.offset(+150, -50),
                terminal.offset(-80, -50),
                terminal.offset(-80, -330),
            ],
            echelon="PL",
            notes="OBJ AMBER NORTH — control tower + admin block (ALPHA-11 1er PLT)",
        )
    )
    ...
    items.append(
        poly(
            "OBJ_AREA",
            [
                hangars.offset(+150, -100),
                hangars.offset(+150, +400),
                hangars.offset(-150, +400),
                hangars.offset(-150, -100),
            ],
            echelon="PL",
            notes="OBJ AMBER EAST — hangars + east taxiway (ALPHA-21 2eme PLT)",
        )
    )
    ...
    items.append(
        poly(
            ...
            [
                rwy_09.offset(+60, +50),
                rwy_09.offset(+60, +600),
                rwy_09.offset(-60, +600),
                rwy_09.offset(-60, +50),
            ],
            echelon="PL",
            notes="OBJ ORANGE WEST — RWY 09 threshold + CLP touchdown zone (BRAVO)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="COY",
            notes="OBJ NASSAU WEST — admin/vehicle park (BRAVO breach W gate)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="COY",
            notes="OBJ NASSAU EAST — barracks/command block (ALPHA breach N gate)",
        )
    )
    ...
    items.append(
        poly(
            ...
            echelon="PL",
            notes="OBJ NASSAU SOUTH — motor pool + EPW holding (CHARLIE secures)",
        )
    )
    ...
    items.append(
        {
            "type": "POI",
            "symbol_code": "SFGPCA------",
            "latitude": uav_pos.lat,
            "longitude": uav_pos.lon,
            "affiliation": "FRIENDLY",
    ...
    ambush_pos = n237_block.offset(
        +200, +900
    )  # ambush on N237 between VCP + Amersfoort
    items.append(
        {
            "type": "ENEMY",
            "symbol_code": "SHGPUCIZ----",
            "latitude": ambush_pos.lat,
            "longitude": ambush_pos.lon,
            "affiliation": "ENEMY",
    ...
    items.append(
        {
            "type": "ENEMY",
            "symbol_code": "SHGPUCFW----",
            "latitude": ambush_pos.offset(+50, +80).lat,
            "longitude": ambush_pos.offset(+50, +80).lon,
            "affiliation": "ENEMY",
    ...
    items.append(
        {
            "type": "ENEMY",
            "symbol_code": "SHGPCA------",
            "latitude": enemy_drone.lat,
            "longitude": enemy_drone.lon,
            "affiliation": "ENEMY",
...
def build_fire_plan() -> list[dict]:
    ...
    return [
        ...
        {
            "latitude": air.offset(-600, +100).lat,
            "longitude": air.offset(-600, +100).lon,
            "altitude": 5.0,
            "direction": 0.0,
            "mission_type": "SUPPRESSION",
            "ammunition": "SMOKE",
        ...
        },
        {
            ...
            "latitude": kaz.offset(+600, 0).lat,
            "longitude": kaz.offset(+600, 0).lon,
            "altitude": 300.0,
            "direction": 180.0,
            "mission_type": "SUPPRESSION",
            "ammunition": "ILLUM",
...
def build_vehicles(
    api: Api, admin_tok: str, by_cs: dict[str, OpRoster], teams: dict[str, int]
) -> dict[str, int]:
    ...
    specs = [
        ...
        _v(
            ...
            "Land Rover Wolf 130",
            "SFGPUCV-----",
            "BN CO vehicle — HAMMER-6; HF/VHF comms fit; BN TAC CP",
            op="HAMMER-6",
        ),
        _v(
            "BN-LR-XO",
            "Land Rover Wolf 130",
            "SFGPUCV-----",
            "BN 2IC vehicle — HAMMER-5; BN advance party Phase 2",
            op="HAMMER-5",
        ),
        _v(
            "BN-LR-SIG",
            "Land Rover Wolf 130",
            "SFGPUCV-----",
            "BN Sigs vehicle — RADIO-6; comms relay + BN CP",
            op="RADIO-6",
        ),
        _v(
            "BN-LR-MED",
            "Land Rover Wolf 130",
            "SFGPUCV-----",
            "BN MO vehicle — DOC-6; Role 1 BAS + CCP coordination",
            op="DOC-6",
        ),
        ...
        _v(
            ...
            "FN MAG + 25mm Oerlikon; 8 pax capacity",
            op="ALPHA-6",
            team="ALPHA-HQ",
        ),
        _v(
            ...
            "YPR-765 AIFV",
            "SFGPUCEV----",
            "ALPHA CIE IFV #2 — ALPHA-11 1er PLT carrier",
            op="ALPHA-11",
            team="A1PLT-HQ",
        ),
        _v(
        ...
        _v(
            ...
            "YPR-765 AIFV",
            "SFGPUCEV----",
            "BRAVO CIE IFV #2 — BRAVO-11 1er PLT carrier",
            op="BRAVO-11",
            team="B1PLT-HQ",
        ),
        _v(
            ...
            "YPR-765 AIFV",
            "SFGPUCEV----",
            "CHARLIE CIE IFV — advance guard vehicle; leads column Phase 2",
            op="CHARLIE-6",
            team="CHARLIE-HQ",
        ),
        _v(
            ...
            "YPR-765 AIFV",
            "SFGPUCEV----",
            "DELTA CIE fire support IFV — AT reserve; MORT-5 FOO on board",
            op="DELTA-6",
            team="DELTA-HQ",
        ),
        ...
        _v(
            ...
            "Jackal 2 MWMIK",
            "SFGPUCV-----",
            "Jackal 1 — JACKAL-1/2 crew; HMG M2 + GPMG; fwd screen N237",
            op="JACKAL-1",
            team="JACKAL-SEC",
        ),
        _v(
            ...
            "[INOPS — awaiting recovery UNIMOG-V3]",
            op="JACKAL-3",
            team="JACKAL-SEC",
        ),
        _v(
            ...
            "Jackal 2 MWMIK",
            "SFGPUCV-----",
            "Jackal 3 — JACKAL-5/6 crew; HMG M2; flanking screen E of Kazerne",
            op="JACKAL-5",
            team="JACKAL-SEC",
        ),
        _v(
            ...
            "Jackal 2 MWMIK",
            "SFGPUCV-----",
            "Jackal 4 — JACKAL-7/8 crew; GPMG; PL CARNATION fwd OP",
            op="JACKAL-7",
            team="JACKAL-SEC",
        ),
        ...
        _v(
            ...
            "SFGPUCV-----",
            "Unimog 1 — ALMA stores/ammo; UNIMOG-1 driver; LOG-6 coord",
            op="UNIMOG-1",
            team="UNIMOG-SEC",
        ),
        _v(
            ...
            "Mercedes Unimog U4000",
            "SFGPUCV-----",
            "Unimog 2 — fuel tanker (1,500L); UNIMOG-2 driver",
            op="UNIMOG-2",
            team="UNIMOG-SEC",
        ),
        _v(
            ...
            "tasked recovery JACKAL-V2 (IED mobility kill N237)",
            op="UNIMOG-3",
            team="UNIMOG-SEC",
        ),
        # ── DUSTOFF helicopter ────────────────────────────────────────────────
        _v(
            ...
            "2 litter + 4 sitting; FOB Gilze-Rijen; DOC-6 coordinates CASEVAC",
            op="DOC-6",
        ),
        # ── Pioneer Unimog ────────────────────────────────────────────────────
        _v(
            ...
            "Mercedes Unimog U4000",
            "SFGPUCV-----",
            "Pioneer vehicle — PION-6/1; EOD/IED kit; pre-CLP route clearance",
            op="PION-6",
            team="PION-ALPHA",
        ),
    ]
...
def submit_logreps(api: Api, by_cs: dict[str, OpRoster], admin_tok: str) -> None:
    ...
    logrep1: dict = {
        ...
        "payload": {
            ...
            "section_c": [
                {
                    ...
                    "type": "Jackal 2 MWMIK",
                    "status": "INOPS",
                    "fault": "IED blast mobility kill — L front axle + engine bay; "
            ...
            "section_d": {
                ...
                "awaiting_evac": 1,
                "recovery_requested": [
                    "JACKAL-V2 (N237 ambush site, grid 31UFT 5760 7810)"
                ],
                "notes": (
            ...
            "section_c": [
                {
                    ...
                    "type": "Jackal 2 MWMIK",
                    "status": "INOPS",
                    "fault": "Recovered by UNIMOG-V3 H+130; "
                ...
                },
                {
                    ...
                    "type": "YPR-765 AIFV",
                    "status": "INOPS",
                    "fault": "RPG strike W gate Kazerne H+172; "
...
def build_cas(api: Api, admin_tok: str, by_cs: dict[str, OpRoster]) -> None:
    ...
    try:
        api.c.post(
            ...
            headers={
                "Authorization": f"Bearer {fire_6.token}",
...
def submit_reports(api: Api, by_cs: dict[str, OpRoster]) -> None:
    ...
    if uav1 and uav1.token:
        ...
        resp = api.c.post(
            ...
            headers={
                "Authorization": f"Bearer {uav1.token}",
                "X-Mission-ID": str(api.mission_id),
            },
        )
        if resp.status_code in (200, 201):
            log.info(
                "DRONE_SPOT filed: UAV-1 — OPFOR ISR quad over N237 (H+80, ambush pre-indicator)"
            )
        else:
    ...
    if j1 and j1.token:
        ...
        log.info(
            "SPOT+TIC filed: JACKAL-1 — OPFOR ambush on N237, IED + PKM + RPG (H+105)"
        )
    ...
    if b112 and b112.token:
        ...
        log.info(
            "MEDEVAC filed: BRAVO-112 PRIORITY (GSW + fragment, Kazerne W gate, H+185)"
        )
    ...
    if doc6 and doc6.token:
        ...
        log.info(
            "CASEVAC SORTIE 1 confirmed: ANGEL 31 picked up ALPHA-112 + JACKAL-3 from LZ MAROON"
        )
        ...
        log.info(
            "CASEVAC SORTIE 2 confirmed: ANGEL 31 picked up BRAVO-112 + BRAVO-113 from LZ TULIP"
        )
...
def simulate_movement(api: Api, steps: int, dt: float) -> None:
    ...
    for r in ROSTER:
        ...
        if cs.startswith("ALPHA"):
            ...
            mid = LL(*N237_BLOCK).offset(
                np // 3, ep // 3
            )  # N237 block (via S perimeter)
    ...
    log.info(
        "H-HOUR %s — BN movement: %d operators / %d steps × %.1fs",
        H_HOUR,
        len(ROSTER),
        steps,
        dt,
    ...
    for i in range(steps + 1):
        ...
        for r in ROSTER:
            ...
            try:
                resp = api.c.post(
                    api._p("/tracking/position"), json=body, headers=api._hdr(r.token)
                )
                if resp.status_code == 401 and r.password:
                    ...
                    try:
                        ...
                        api.c.post(
                            api._p("/tracking/position"),
                            json=body,
                            headers=api._hdr(r.token),
                        )
                    except Exception:
                        r.token = ""
            except Exception as exc:
        ...
        if i % 10 == 0:
            phase = (
                "PHASE 1 — airborne assault on Soesterberg"
                if t < 0.45
                else (
                    "PHASE 1-2 — consolidation + CLP fly-in"
                    if t < 0.65
                    else "PHASE 2 — mounted assault on Kazerne"
                )
            )
...
def reset_objects(api: Api, admin_tok: str) -> None:
    ...
    deleted = sum(
        1 for o in objs if api.delete(f"/tactical-objects/{o['id']}", admin_tok) == 204
    )
...
def reset_vehicles(api: Api, admin_tok: str) -> None:
    ...
    deleted = sum(
        1 for v in vehs if api.delete(f"/vehicles/{v['id']}", admin_tok) == 204
    )  # type: ignore[index]
    log.info("reset vehicles: deleted %d", deleted)


def _build_by_cs_from_db(
    api: Api, admin_tok: str, teams: dict[str, int]
) -> dict[str, "OpRoster"]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--reset", action="store_true", help="Delete all tactical objects before run"
    )
    parser.add_argument(
        "--reset-vehicles", action="store_true", help="Delete all vehicles before run"
    )
    parser.add_argument("--no-move", action="store_true", help="Skip GPS simulation")
    parser.add_argument(
        "--no-reports", action="store_true", help="Skip MEDEVAC/SPOT/CAS/LOGREP"
    )
    ...
    api.create_mission(
        ...
        map_center_lat=52.1400,
        map_center_lng=5.3000,
        map_zoom=12,
    )
    ...
    if args.loop:
        log.info(
            "LOOP MODE — Ctrl-C to stop  |  steps=%d  dt=%.2fs", args.steps, args.dt
        )
        ok = _login_all(args.op_password)
        log.info(
            "initial login: %d/%d operators  (tokens refresh on 401 only)",
            ok,
            len(ROSTER),
        )
        iteration = 0
    ...
    graphics = build_graphics(sim_utils.mgrs)
    posted = sum(
        1 for g in graphics if _try_post(api, "/tactical-objects", admin_tok, g)
    )
    log.info("tactical graphics posted: %d/%d", posted, len(graphics))
    ...
    if not args.no_reports:
        ...
        submit_logreps(api, by_cs, admin_tok)
        log.info(
            "all reports filed: 3× MEDEVAC, 2× SPOT, 1× DRONE_SPOT, 2× CAS, 2× LOGREP"
        )
