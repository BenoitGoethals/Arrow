...
parser.add_argument("--delay", type=float, default=0.3,
                    help="Seconds between each POST (default 0.3)")
parser.add_argument("--count", type=int, default=10,
                    help="Number of LOGREPs to send (default 10)")
...
def make_logrep(i: int) -> dict:
    ...
    logreqs = []
    if class_i["dos"] < 1.5:
        logreqs.append({"priority": "URGENT",  "item": "24HR ORP x40",  "quantity": 40,  "eta": "AS SOON AS POSSIBLE"})
    if class_iii["diesel_litres"] < 400:
        logreqs.append({"priority": "ROUTINE", "item": "DIESEL JERRY CAN x20", "quantity": 20, "eta": "NLT 24H"})
    if section_d["deadline_equipment"] > 1:
        logreqs.append({"priority": "PRIORITY", "item": "TRACK LINK SETS", "quantity": 2, "eta": "NLT 12H"})
    if not logreqs:
        logreqs.append({"priority": "ROUTINE", "item": "NO REQUEST", "quantity": 0, "eta": ""})

    # Section G — assessment
    fmc_ratio = sum(v["fmc"] for v in section_c) / max(1, sum(v["on_hand"] for v in section_c))
...
def main() -> None:
    ...
    with httpx.Client(timeout=15, verify=False) as client:
        ...
        r = client.post(f"{base}/auth/login",
                        data={"username": "benoit", "password": "ranger14"})
        ...
        for i in range(args.count):
            ...
            if r.status_code == 201:
                rep = r.json()
                log.info("  [%2d/%d] %-18s  serial=%-22s  id=%d  assessment=%s",
                         i + 1, args.count, unit,
                         payload["serial"], rep["id"],
                         payload["section_g"]["assessment"])
                success += 1
            else:
                log.error("  [%2d/%d] %-18s  FAILED %d %s",
                          i + 1, args.count, unit, r.status_code, r.text[:120])

            if args.delay > 0 and i < args.count - 1:
                time.sleep(args.delay)

        log.info("Done — %d/%d LOGREPs submitted (mission_id=%s)",
                 success, args.count, mission_id)
