...
def _draw_chrome(canv: Canvas, doc: BaseDocTemplate, slide_no: int) -> None:
    ...
    canv.setFillColor(LIGHT)
    canv.setFont("Helvetica", 9)
    canv.drawRightString(PAGE_W - MARGIN, PAGE_H - 0.62 * cm, f"{slide_no:02d} / 10")
    # Footer rule
    ...
    canv.drawRightString(
        PAGE_W - MARGIN, MARGIN - 0.9 * cm, "github.com/BenoitGoethals/Arrow"
    )
    canv.restoreState()
...
class SlideTemplate(PageTemplate):
    ...
    def beforeDrawPage(self, canv, doc):
        if self.hero:
            ...
            canv.line(PAGE_W - 9.5 * cm, PAGE_H / 2, PAGE_W - 1.5 * cm, PAGE_H / 2)
            canv.setFillColor(LIGHT)
            canv.setFont("Helvetica-Bold", 11)
...
def _metric(value: str, label: str) -> Table:
    t = Table(
        [[Paragraph(value, S["metric_n"])], [Paragraph(label, S["metric_l"])]],
        colWidths=[5.6 * cm],
        rowHeights=[1.8 * cm, 0.7 * cm],
    )
    t.setStyle(
        TableStyle(
            [
...
def _two_col(left_flowables, right_flowables, ratio=(1, 1)) -> Table:
    ...
    t = Table([[left_flowables, right_flowables]], colWidths=[w_l, w_r])
    t.setStyle(
        TableStyle(
            [
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def slide_cover() -> list:
    return [
        ...
        Paragraph(
            ...
            ParagraphStyle(
                "kicker", fontName="Helvetica", fontSize=13, textColor=LIGHT, leading=18
            ),
        ),
    ]
...
def slide_problem() -> list:
    body = [
        ...
        Spacer(1, 2 * mm),
        Paragraph(
            "Commercial SA stacks solved this on paper a decade ago. "
            "In practice, small units still face four hard frictions.",
            S["body"],
        ),
        Spacer(1, 4 * mm),
        Paragraph("Fragmentation.", S["h2"]),
        Paragraph(
            "Map, chat, fires, the OPORD PDF and the contact-report "
            "notebook never live in the same place. Every cross-domain "
            "hand-off is a screenshot or a verbal repeat over the net.",
            S["body"],
        ),
        Paragraph("Closed ecosystems.", S["h2"]),
        Paragraph(
            "Vendor lock-in, opaque protocols, hardware you can't audit, "
            "license counts that don't survive a battalion rotation.",
            S["body"],
        ),
        Paragraph("Slow OODA.", S["h2"]),
        Paragraph(
            "OPORDs live in PDFs. 9-Lines live on radios. The "
            "common operating picture is rarely common, rarely current.",
            S["body"],
        ),
        Paragraph("Hard to interoperate.", S["h2"]),
        Paragraph(
            "ATAK speaks CoT, NATO speaks APP-11, partners speak "
            "what they were issued. Bridging them is a project, not a feature.",
            S["body"],
        ),
    ]
    return body
...
def slide_what_arrow_is() -> list:
    left = [
        ...
        Paragraph(
            "A modern soldier-system platform — open, inspectable, "
            "doctrinally honest.",
            S["body"],
        ),
        Spacer(1, 4 * mm),
        ...
        Paragraph(
            "FastAPI backend — JWT auth, SQLAlchemy, WebSocket pub/sub.", S["bullet"]
        ),
        Paragraph(
            "Flask Battle-Captain dashboard — live map, OPORD editor, FDC.", S["bullet"]
        ),
        Paragraph(
            "Android Jetpack-Compose client — OSMdroid, milsymbol, foreground GPS.",
            S["bullet"],
        ),
        Spacer(1, 3 * mm),
        Paragraph(
            "Every domain is a SOLID, pluggable module: auth, tracking, "
            "alerts, messaging, reports, fires, OPORDs, battle "
            "management, CoT, photos, streams.",
            S["body"],
        ),
    ]
    right = [
        ...
        Spacer(1, 4 * mm),
        Paragraph(
            "Built in Python 3.14, Kotlin, modern web — no Electron, "
            "no native blobs you can't read.",
            S["body"],
        ),
    ]
...
def slide_capabilities() -> list:
    rows = [
        [
            "Live tracking",
            "Operator GPS · online/offline · role-aware · 90-s heartbeat.",
        ],
        [
            "MIL-STD-2525",
            "Friendly blue / enemy red / unknown yellow · all tactical control graphics drawable as either side · echelon designators.",
        ],
        [
            "OPORDs",
            "Full five-paragraph NATO/US Operation Orders · server-rendered map snapshots with overlays · PDF export · sendable to operators · readable on Android.",
        ],
        [
            "Tactical messaging",
            "Direct / group / broadcast · photo attachments · auto-dismiss notifications · 10-min chat toasts with inline reply.",
        ],
        [
            "Reports",
            "Contact / Spot / CASEVAC / MEDEVAC / CAS 9-Liners · reviewer workflow · NATO CBRN 1–6 parser.",
        ],
        [
            "Fire missions",
            "Call-for-fire · FDC dashboard · L16 81 mm firing solutions (charge, QE, deflection, TOF).",
        ],
        [
            "Alerts",
            "TIC · MEDICAL · EVAC · LOST_COMMS · one-tap broadcast · audio + visual escalation.",
        ],
        [
            "Streams",
            "Operator camera → company TOC · MJPEG over WebSocket · persisted recordings.",
        ],
        [
            "CoT bridge",
            "Cursor-on-Target XML in/out — interop with ATAK and partner systems.",
        ],
        [
            "Security",
            "JWT + auto-secret · TOTP MFA · AES-256-GCM photo encryption at rest · NIST CSF audit log · rate limiting · CSP.",
        ],
    ]
    body = [Paragraph("What ARROW does today.", S["h1"]), Spacer(1, 3 * mm)]
    data = [
        [Paragraph(f"<b>{k}</b>", S["body"]), Paragraph(v, S["body"])] for k, v in rows
    ]
...
def slide_diff() -> list:
    ...
    body = [Paragraph("What makes it different.", S["h1"]), Spacer(1, 3 * mm)]
    data = []
    for title, txt in items:
        data.append(
            [
                Paragraph("<font color='#FBBF24'>■</font>", S["body"]),
...
def slide_scenario() -> list:
    body = [
        ...
        Spacer(1, 3 * mm),
        Paragraph("A platoon leader with ARROW has <b>one screen</b> to:", S["body"]),
...
def slide_architecture() -> list:
    body = [
        ...
        Paragraph(
            ...
            "SQLAlchemy on PostgreSQL 16 + PostGIS 3.4; MapServer (OGC WMS/WFS). One <i>broadcaster</i> "
...
def slide_roadmap() -> list:
    items = [
        (
            "Multi-domain interop",
            "Full CoT 2.0 · OGC GeoPackage · NATO ADatP-3 · OpenC2.",
        ),
        (
            "Battlefield AI",
            "Snapshot → automatic symbol recognition; chat → automatic SITREP draft; sensor fusion across operator feeds.",
        ),
        (
            "Federation",
            "Mesh of company TOCs gossiping state — disconnected ops, autonomous reconnect.",
        ),
        (
            "Coalition profile",
            "Per-partner data partitions · ROE-aware sharing · automatic redaction.",
        ),
        (
            "Hardware",
            "Reference deployments on EUD, Samsung S25 Tactical, Boxer-class command vehicles.",
        ),
    ]
    body = [Paragraph("Where it goes next.", S["h1"]), Spacer(1, 4 * mm)]
    for title, txt in items:
...
def slide_ask() -> list:
    body = [
        ...
        Spacer(1, 4 * mm),
        Paragraph(
            "Give us this — we give you a SA platform your operators "
            "actually want to use, on infrastructure you own.",
            S["body"],
        ),
        Spacer(1, 6 * mm),
    ]
...
def slide_thanks() -> list:
    return [
        ...
        Paragraph(
            ...
            ParagraphStyle(
                "link", fontName="Courier", fontSize=14, textColor=ACCENT, leading=18
            ),
        ),
        Paragraph(
            "benoit.goethals@gmail.com",
            ParagraphStyle(
                "mail", fontName="Helvetica", fontSize=12, textColor=LIGHT, leading=18
            ),
        ),
    ]
...
def build(path: Path) -> None:
    doc = BaseDocTemplate(
        ...
        title="ARROW — Pitch",
        author="ARROW",
        subject="Soldier System Platform",
    )
    ...
    doc.addPageTemplates(
        [SlideTemplate(slide_no=i + 1, hero=hero) for i, (_, hero) in enumerate(slides)]
    )
