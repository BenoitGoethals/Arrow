...
SOF_ELEMENTS = [
    ...
    {"key": "CBRN", "name": "CBRN", "icon": "☢", "color": "#d2a8ff", "roles": {"CBRN"}},
    {
        "key": "LOG",
        "name": "SUSTAINMENT",
        "icon": "📦",
...
class StrikePlannerWindow(QMainWindow):
    ...
    def _build_task_org(self, b: dict) -> QWidget:
        ...
        if not operators:
            operators = [
                {"id": oid, "callsign": f"OP-{oid}", "role": "OPERATOR"}
                for oid in (b.get("operator_ids") or [])
            ]

        lay.addWidget(
            _section_header(f"TASK ORGANIZATION  —  {len(operators)} OPERATORS")
        )
        ...
        for elem in SOF_ELEMENTS:
            ...
            for op in ops:
                ...
                online = (
                    "●" if op.get("online") or op.get("status") == "ONLINE" else "○"
                )
                clr = "#3fb950" if online == "●" else "#6e7681"
                row = QHBoxLayout()
                row.setContentsMargins(16, 1, 4, 1)
    ...
    def _build_fire_plan(self, b: dict) -> QWidget:
        ...
        if not fire_missions:
            ...
            if fm_ids:
                fire_missions = [
                    {"id": fid, "mission_type": "PLANNED", "status": "PENDING"}
                    for fid in fm_ids
                ]
        ...
        if fire_missions:
            ...
            for fm in fire_missions:
                ...
                c = (
                    ...
                    else "#d29922" if fm.get("status") == "PENDING" else "#c9d1d9"
                )
    ...
    def _build_exfil(self, b: dict) -> QWidget:
        ...
        if routes:
            for r in routes:
                ...
                hdr.setStyleSheet(
                    f"color:{color};font-size:11px;font-weight:700;padding:5px 0 2px;"
                )
                lay.addWidget(hdr)
...
def _key(text: str) -> QLabel:
    l = QLabel(text)
    l.setStyleSheet(
        "color:#6e7681;font-size:9px;font-weight:700;letter-spacing:1px;min-width:140px;"
    )
    return l
...
def _asset_card(
    title: str, subtitle: str, fields: list[tuple[str, str]], color: str
) -> QFrame:
    card = QFrame()
    card.setStyleSheet(
    ...
    for k, v in fields:
        if v and v != "—":
            row = QHBoxLayout()
            row.setSpacing(8)
            row.addWidget(_key(k))
            row.addWidget(_val(v), 1)
            lay.addLayout(row)
    return card
