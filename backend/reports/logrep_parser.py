...
def _parse_perstat(block: str) -> dict:
    return {
        ...
        "wia_evacuated": _int(
            block, "WIA EVAC", "WIA-EVAC", "WIAEVAC", "WIA(E)", "EVACUATED"
        ),
...
def _parse_supply(block: str) -> dict:
                    ...
                    ammo.append(
                        {
                            "type": parts[0],
                            "on_hand": (
                                int(re.sub(r"[^\d]", "", parts[1]))
                                if len(parts) > 1
                                else 0
                            ),
                            "consumption": (
                                int(re.sub(r"[^\d]", "", parts[2]))
                                if len(parts) > 2
                                else 0
                            ),
                        }
    ...
    return {
        ...
        "class_ix_shortfalls": _kv(
            block, "IX SHORTFALL", "PARTS SHORTFALL", default=""
        ),
    }
...
def _parse_medical(block: str) -> dict:
    return {
        ...
        "medic_operational": not bool(
            re.search(r"medic.*not.*oper|medic.*non-oper", block, re.I)
        ),
        "supplies_critical": bool(
            re.search(r"suppl.*critical|critical.*suppl", block, re.I)
        ),
...
def parse_logrep_cot(xml_text: str) -> dict | None:
    ...
    if supply is not None:
        ...
        section_b = {
            ...
            "class_iii": {
                "diesel_litres": (
                    _af(ciii, "diesel_litres") if ciii is not None else 0.0
                ),
                "petrol_litres": (
                    _af(ciii, "petrol_litres") if ciii is not None else 0.0
                ),
    ...
    if med is not None:
        section_e = {
            ...
            "medic_operational": med.get("medic_operational", "true").lower()
            != "false",
            "supplies_critical": med.get("supplies_critical", "false").lower()
            == "true",
    ...
    if assess is not None:
        section_g = {
            "assessment": assess.get(
                "status", assess.get("assessment", "AMBER")
            ).upper(),
