...
_DRAG_SCALE: dict[int, float] = {
    c: r_max / (v * v / _GRAVITY)
    for c, v, r_max in _CHARGES
}

# Min ranges per charge (rough — high-angle solution at QE≈1480 mils).
# Well below this and you must drop a charge.
_MIN_RANGE: dict[int, float] = {1: 80, 2: 200, 3: 350, 4: 500,
                                5: 700, 6: 900, 7: 1100, 8: 1300, 9: 1500}
...
def _bearing_range_m(gun_lat: float, gun_lon: float,
    ...
    dlon = (tgt_lon - gun_lon) * 111_000.0 * math.cos(math.radians((gun_lat + tgt_lat) / 2))
...
def _offset_by(lat: float, lon: float, az_mils: float, dist_m: float) -> tuple[float, float]:
...
def solve_l16_81mm(
    *,
    gun_lat: float, gun_lon: float, gun_alt_m: float,
    ...
    if charge is None:
        return FiringSolution(
            charge=0, qe_mils=0, defl_mils=0, tof_s=0.0, mv_ms=0.0,
            range_m=rng_m, azimuth_mils=int(az_mils),
            impact_lat=tgt_lat, impact_lon=tgt_lon,
            error=f"target {rng_m:.0f} m exceeds max range (5650 m)",
        )
    if rng_m < _MIN_RANGE.get(charge, 0):
        return FiringSolution(
            charge=charge, qe_mils=0, defl_mils=0, tof_s=0.0, mv_ms=0.0,
            range_m=rng_m, azimuth_mils=int(az_mils),
            impact_lat=tgt_lat, impact_lon=tgt_lon,
    ...
    if qe is None:
        return FiringSolution(
            charge=charge, qe_mils=0, defl_mils=0, tof_s=0.0, mv_ms=mv,
            range_m=rng_m, azimuth_mils=int(az_mils),
            impact_lat=tgt_lat, impact_lon=tgt_lon,
    ...
    return FiringSolution(
        charge=charge, qe_mils=int(round(qe)), defl_mils=int(round(defl)),
        tof_s=round(tof, 1), mv_ms=mv, range_m=round(rng_m, 1),
    ...
    if pattern == "LINE":
        ...
        for i in range(total):
            if total == 1:
                ...
                t = i / (total - 1)
            impacts.append((s_lat + t * (e_lat - s_lat),
                            s_lon + t * (e_lon - s_lon)))
        return impacts
...
def solve_mortar_plan(
    *,
    ...
    impacts = build_sheaf(pattern=pattern, target=target,
                          gun_count=len(guns), rounds_per_gun=rounds_per_gun)
    ...
    for gi, g in enumerate(guns):
        ...
        for tgt in my_impacts:
            sol = solve_l16_81mm(
                gun_lat=glat, gun_lon=glon, gun_alt_m=galt,
                tgt_lat=tgt[0], tgt_lon=tgt[1], tgt_alt_m=target_alt_m,
