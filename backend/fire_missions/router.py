...
def _validate_mortar_plan(
    plan: MortarPlanIn,
) -> tuple[Literal["POINT", "LINE", "ZONE"], str]:
    ...
    return cast(Literal["POINT", "LINE", "ZONE"], pattern), ammo
...
@router.post(
    "/mortar", response_model=FireMissionOut, status_code=status.HTTP_201_CREATED
)
async def submit_mortar(
    ...
    if not impacts:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY, "no impact points produced"
        )
    ...
    g0 = result["guns"][0]
    direction = (
        (g0["solutions"][0]["azimuth_mils"] * 360.0 / 6400.0)
        if g0["solutions"]
        else 0.0
    )
