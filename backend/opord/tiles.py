...
TILE_URL = os.environ.get(
    "ARROW_TILE_URL", "https://tile.openstreetmap.org/{z}/{x}/{y}.png"
)
...
def _affiliation_style(aff: str) -> tuple[tuple[int, int, int], tuple[int, int, int]]:
    if aff == "ENEMY":
        return ((220, 38, 38), (127, 29, 29))  # red
    if aff == "UNKNOWN":
        return ((250, 204, 21), (113, 63, 18))  # yellow
...
def _load_font(size: int) -> ImageFont.ImageFont:
    ...
    for c in candidates:
        if os.path.exists(c):
            try:
                return ImageFont.truetype(c, size)  # type: ignore[return-value]
            except Exception:
                pass
...
def _draw_overlays(
    canvas: Image.Image, db: Session, bbox: list[float], z: int, x0: int, y0: int
) -> None:
    s, w, n, e = bbox
    ...
    objects: Iterable[TacticalObject] = (
        ...
        .filter(
            TacticalObject.latitude >= s,
            TacticalObject.latitude <= n,
            TacticalObject.longitude >= w,
            TacticalObject.longitude <= e,
        )
        .all()
    )
    for to in objects:
        if (to.type or "") in TG_TYPES:
            fill, outline = _affiliation_style(
                getattr(to, "affiliation", "FRIENDLY") or "FRIENDLY"
            )
        else:
    ...
    for op in operators:
        if not _in_bbox(op.latitude, op.longitude, bbox):  # type: ignore[arg-type]
            continue
        px, py = _project(op.latitude, op.longitude, z, x0, y0)  # type: ignore[arg-type]
        r = 6
        draw.ellipse(
            (px - r, py - r, px + r, py + r),
            fill=OPERATOR_FILL + (255,),
            outline=OPERATOR_OUTLINE + (255,),
            width=2,
        )
        draw.text(
            (px + r + 2, py - r - 2),
    ...
    for al in alerts:
        if not _in_bbox(al.latitude, al.longitude, bbox):  # type: ignore[arg-type]
            continue
        px, py = _project(al.latitude, al.longitude, z, x0, y0)  # type: ignore[arg-type]
