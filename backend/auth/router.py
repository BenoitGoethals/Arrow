...
def register(request: Request, payload: RegisterIn, db: Session = Depends(get_db)) -> TokenOut:
    result = auth_service.register_operator(
        db, callsign=payload.callsign, password=payload.password,
        rank=payload.rank, team_id=payload.team_id, ip=_client_ip(request),
    )
...
def register_elevated(
    request: Request, payload: RegisterIn,
    db: Session = Depends(get_db),
    current: Operator = Depends(require_role("ADMIN")),
) -> TokenOut:
    result = auth_service.register_elevated(
        db, current_id=current.id,
        callsign=payload.callsign, password=payload.password,
        rank=payload.rank, role=payload.role, team_id=payload.team_id,
...
def login(
    ...
    result = auth_service.login(
        db, username=form.username, password=form.password, ip=_client_ip(request),
    )
    return _token_out(result)
...
def mfa_verify(request: Request, payload: MfaVerifyIn,
    ...
    result = mfa_service.verify_second_step(
        db, mfa_session=payload.mfa_session, code=payload.code, ip=_client_ip(request),
    )
    return _token_out(result)
