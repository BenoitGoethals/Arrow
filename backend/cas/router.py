...
from backend.api.schemas import (
    CasAssetIn, CasAssetOut, CasAssetUpdate,
    CasNineLinerIn, ReportOut,
)
...
def list_assets(
    ...
    q = db.query(CasAsset).order_by(CasAsset.available_from.nulls_last(), CasAsset.callsign)
    if mission:
        q = q.filter(
...
async def create_asset(
    ...
    log_event(db, "CAS_ASSET_CREATED", operator_id=current.id,
...
async def update_asset(
    ...
    if body.callsign      is not None: asset.callsign      = body.callsign
    if body.aircraft_type is not None: asset.aircraft_type = body.aircraft_type
    if body.ordnance      is not None: asset.ordnance      = body.ordnance
    if body.frequency     is not None: asset.frequency     = body.frequency
    if body.notes         is not None: asset.notes         = body.notes
    if body.available_from is not None: asset.available_from = body.available_from
    if body.available_to   is not None: asset.available_to   = body.available_to
    ...
    log_event(db, "CAS_ASSET_UPDATED", operator_id=current.id,
...
async def delete_asset(
    ...
    log_event(db, "CAS_ASSET_DELETED", operator_id=current.id,
              resource=f"cas_asset:{asset_id}")
...
async def submit_cas_request(
    ...
    if body.fo_operator_id is not None:
        ...
        if not fo:
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "FO operator not found")
    ...
    log_event(db, "CAS_REQUEST_SUBMITTED", operator_id=current.id,
