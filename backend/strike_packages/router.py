...
async def activate_package(
    ...
    if pkg.status not in ("PLANNING",):
        raise HTTPException(status.HTTP_409_CONFLICT, f"Cannot activate package in status {pkg.status}")
    pkg.status = "ACTIVE"
    db.commit()
    db.refresh(pkg)
...
def bundle(
    ...
    def _op(op: Operator) -> dict:
        return {"id": op.id, "callsign": op.callsign, "rank": op.rank,
                "role": op.role, "status": op.status,
                "latitude": op.latitude, "longitude": op.longitude}

    def _to(t: TacticalObject) -> dict:
        return {"id": t.id, "type": t.type, "symbol_code": t.symbol_code,
                "latitude": t.latitude, "longitude": t.longitude,
                "notes": t.notes, "geometry": t.geometry,
                "affiliation": t.affiliation, "echelon": t.echelon,
                "rotation": t.rotation}

    def _fm(f: FireMission) -> dict:
        return {"id": f.id, "latitude": f.latitude, "longitude": f.longitude,
                "mission_type": f.mission_type, "ammunition": f.ammunition,
    ...
    for to_id in to_ids:
        ...
        if tobj:
            events.append(CotEvent(
                ...
                lat=tobj.latitude,
                lon=tobj.longitude,
                callsign=f"{tobj.type}: {tobj.notes[:40]}" if tobj.notes else tobj.type,
            ).to_xml())
