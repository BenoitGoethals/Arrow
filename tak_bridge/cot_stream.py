...
class CotEvent:
    ...
    def to_xml(self) -> bytes:
        ...
        if self.speed or self.course:
            etree.SubElement(
                det, "track", speed=f"{self.speed:.2f}", course=f"{self.course:.1f}"
            )
        if self.team:
            etree.SubElement(
                det, "__group", role=self.role or "Team Member", name=self.team
            )
        etree.SubElement(
            det, "takv", os="0", version="4.10", device="", platform=self.platform
        )
...
def tacobj_to_cot(obj: dict) -> CotEvent | None:
    ...
    cot_type = sidc_cot.get(sidc[:12].upper() if sidc else "", f"a-{aff}-G")
