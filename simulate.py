...
parser.add_argument(
    "--reset", action="store_true", help="Delete all sim operators before starting"
)
parser.add_argument(
    "--seed-admin",
    default="benoit",
...
_EVENTS = [
    ...
    dict(kind="report", type="SPOT", sidc="", weight=2, notes=""),
]
...
async def bootstrap(
    client: httpx.AsyncClient, all_ops: list[SimOp], sections: list[SimSection]
) -> str:
    """
    ...
    if not sim_admin_token:
        ...
        if row and row.get("role") != "ADMIN":
            ...
            if patched:
                log.info(
                    "Promoted %s to ADMIN (was %s)", admin_op.callsign, row.get("role")
                )
                # Re-login to obtain a token with the elevated role claim.
                sim_admin_token = (
                    await login(client, admin_op.callsign) or sim_admin_token
                )
...
async def run_section(
    client: httpx.AsyncClient, sec: SimSection, speed_mult: float, stagger_s: float
) -> None:
    ...
    while True:
        ...
        while True:
            ...
            centre_lat, centre_lon = (sec.operators[0].lat, sec.operators[0].lon)
            new_clat, new_clon = step_towards(
                ...
                target_lon,
                speed,
                real_dt * speed_mult,  # real distance in real dt
            )
...
async def run_command(
    client: httpx.AsyncClient, all_ops: list[SimOp], speed_mult: float
) -> None:
...
async def run_enemy_marker(
    client: httpx.AsyncClient, sections: list[SimSection], speed_mult: float
) -> None:
    ...
    while True:
        ...
        live_sections = [
            s for s in sections if any(o.token and o.lat != 0 for o in s.operators)
        ]
        ...
        if op.token:
            ...
            content = msg_map.get(evt["type"], f"CONTACT — {evt['type']} grid {grid}")
            await api(
                client,
                "POST",
                "/messages",
...
_AGENT_CATALOGUE: list[tuple[str, str, str, str]] = [
    ...
    (
        ...
        "CBRN_3",
        "DIRTY BOMB",
        "Radiological dispersal device — gamma rate elevated",
    ),
...
async def run_cbrn_worldwide(
    client: httpx.AsyncClient, admin_op: SimOp, speed_mult: float
) -> None:
    ...
    while True:
        ...
        if r:
            log.info(
                "☣️  CBRN %s  %-18s  near %-12s  %.4f,%.4f  wind %d°@%dkm/h  (#%d)",
                msg_type,
                agent_label,
                near,
                lat,
...
def _operation_graphics() -> list[dict]:
    ...
    out.append(
        {
            "type": "OBJ_AREA",
            "latitude": poly_obj[0][0],
            "longitude": poly_obj[0][1],
            "echelon": "COY",
            "notes": "OBJ EAGLE — Delta Coy company objective",
            "geometry": json.dumps(
                {"type": "polygon", "coords": [list(p) for p in poly_obj]}
            ),
        }
    )
    ...
    out.append(
        {
            ...
            "geometry": json.dumps({"type": "line", "coords": [list(p) for p in flot]}),
        }
    )

    # ── FLET — estimated enemy forward line ──
    flet = [
        (_FLET_LAT, _WEST_LON - 0.005),
    ...
    out.append(
        {
            ...
            "geometry": json.dumps({"type": "line", "coords": [list(p) for p in flet]}),
        }
    )
    ...
    out.append(
        {
            "type": "PHASE_LINE",
            "latitude": pl_alpha[0][0],
            "longitude": pl_alpha[0][1],
            "notes": "PL ALPHA — Line of Departure (H-hour)",
            "geometry": json.dumps(
                {"type": "line", "coords": [list(p) for p in pl_alpha]}
            ),
        }
    )
    out.append(
        {
            "type": "PHASE_LINE",
            "latitude": pl_bravo[0][0],
            "longitude": pl_bravo[0][1],
            "notes": "PL BRAVO — Assault Position",
            "geometry": json.dumps(
                {"type": "line", "coords": [list(p) for p in pl_bravo]}
            ),
        }
    )

    # ── Boundaries between platoons ──
    for lon, label in ((_BOUND_W, "BRAVO // ALPHA"), (_BOUND_E, "ALPHA // CHARLIE")):
        out.append(
            {
                "type": "BOUNDARY",
                "echelon": "PL",
                ...
                "geometry": json.dumps(
                    {"type": "line", "coords": [[_LD_LAT, lon], [_FLET_LAT, lon]]}
                ),
            }
        )
    ...
    out.append(
        {
            ...
            "echelon": "PL",
            "rotation": 45,
            "latitude": _LD_LAT + 0.0010,
            "longitude": _CENTRE_LON - 0.0030,
    ...
    out.append(
        {
            ...
            "echelon": "COY",
            "rotation": 90,
            "latitude": _OP_CENTER[0],
            "longitude": _EAST_LON + 0.0030,
...
_OP_ENEMY_PLANT: list[dict] = [
    dict(
        ...
        lat=51.0370,
        lon=4.0995,
        notes="EN platoon — defending Scheldebrug",
    ),
    dict(
        type="INFANTRY",
        sidc="SHGPUCI-----",
        lat=51.0372,
        lon=4.1015,
        notes="EN platoon — Bogaerdpark north edge",
    ),
    dict(
        type="INFANTRY",
        sidc="SHGPUCI-----",
        lat=51.0375,
        lon=4.1075,
        notes="EN section — NE industrial",
    ),
    dict(
        type="ARMOR",
        sidc="SHGPUCA-----",
        lat=51.0388,
        lon=4.1015,
        notes="EN T-72 in hull-down position",
    ),
    dict(
        type="ARTILLERY",
        sidc="SHGPUCF-----",
        lat=51.0410,
        lon=4.1010,
        notes="EN 120 mm mortar baseplate, est.",
    ),
    dict(
        type="AIR_DEFENSE",
        sidc="SHGPUCD-----",
        lat=51.0395,
        lon=4.1100,
        notes="EN MANPADS / ZU-23 covering NE",
    ),
    dict(
        type="SNIPER",
        sidc="SHGPUCIS----",
        lat=51.0365,
        lon=4.1035,
        notes="EN sniper team — church tower",
    ),
    dict(
        type="VEHICLE",
        sidc="SHGPEV------",
        lat=51.0380,
        lon=4.0985,
        notes="Technical w/ HMG — west bank",
    ),
    dict(
        type="UNKNOWN",
        sidc="SUGPU-------",
        lat=51.0405,
        lon=4.0980,
        notes="Unknown contact — recce reports",
    ),
    dict(
        type="POI",
        sidc="SNGPI-------",
        lat=51.0260,
        lon=4.1015,
        notes="POI — fuel station, RV for resupply",
    ),
]
...
_FRIENDLY_BN_LAYDOWN: list[tuple[str, str, float, float, str]] = [
    ...
    ("HHC", "SFGPUCI--H-E", -0.0080, -0.0250, "Headquarters & Headquarters Company"),
    ("WPNS CO", "SFGPUCFH--HE", -0.0080, 0.0250, "Weapons / Heavy Mortar Company"),
    # ── 12× rifle platoons (3 per CO) ────────────────────────────────────
    # A CO platoons
    ("A/1 PL", "SFGPUCI----D", 0.0420, -0.0520, "A Coy / 1 Pl rifle platoon"),
    ("A/2 PL", "SFGPUCI----D", 0.0420, -0.0450, "A Coy / 2 Pl rifle platoon"),
    ("A/3 PL", "SFGPUCIZ---D", 0.0420, -0.0380, "A Coy / 3 Pl mech-infantry platoon"),
    # B CO platoons
    ("B/1 PL", "SFGPUCI----D", 0.0420, -0.0220, "B Coy / 1 Pl rifle platoon"),
    ("B/2 PL", "SFGPUCI----D", 0.0420, -0.0150, "B Coy / 2 Pl rifle platoon"),
    ("B/3 PL", "SFGPUCIZ---D", 0.0420, -0.0080, "B Coy / 3 Pl mech-infantry platoon"),
    # C CO platoons
    ("C/1 PL", "SFGPUCI----D", 0.0420, 0.0080, "C Coy / 1 Pl rifle platoon"),
    ("C/2 PL", "SFGPUCI----D", 0.0420, 0.0150, "C Coy / 2 Pl rifle platoon"),
    ("C/3 PL", "SFGPUCIZ---D", 0.0420, 0.0220, "C Coy / 3 Pl mech-infantry platoon"),
    # D CO platoons
    ("D/1 PL", "SFGPUCI----D", 0.0420, 0.0380, "D Coy / 1 Pl rifle platoon"),
    ("D/2 PL", "SFGPUCI----D", 0.0420, 0.0450, "D Coy / 2 Pl rifle platoon"),
    ("D/3 PL", "SFGPUCIZ---D", 0.0420, 0.0520, "D Coy / 3 Pl mech-infantry platoon"),
    ...
    ("D/1/1 SEC", "SFGPUCI----C", 0.0480, 0.0360, "D Coy / 1 Pl / 1 Sec rifle section"),
    ("D/1/2 SEC", "SFGPUCI----C", 0.0480, 0.0400, "D Coy / 1 Pl / 2 Sec rifle section"),
    ...
    ("MOR PL", "SFGPUCF----D", 0.0070, 0.0080, "Battalion mortar platoon (81 mm)"),
    (
        "AD PL (STINGER)",
        "SFGPUCD----D",
        0.0050,
    ...
    ("SUP CO", "SFGPUSS----E", -0.0150, 0.0150, "Supply company — combat trains"),
    (
        "MAINT SEC",
        "SFGPUSM----C",
        -0.0150,
...
async def plant_friendly_battalion(client: httpx.AsyncClient, admin_op: SimOp) -> None:
    ...
    log.info(
        "── Planting friendly BN laydown — %d unit markers " "(SEC/PL/COY/BN) ─────",
        len(items),
    )
    n_ok = 0
    for item in items:
        r = await api(
            client, "POST", "/tactical-objects", token=admin_op.token, json=item
        )
        if r:
            n_ok += 1
...
async def plant_operation(client: httpx.AsyncClient, admin_op: SimOp) -> None:
    ...
    log.info(
        "── Planting Operation IRON ARDENNES — %d objects across %d "
        "company plans ─────",
        len(all_items),
        len(ARDENNES_PLANS),
    )
    ...
    for item in all_items:
        ...
        if r:
            ...
            log.info("  %s %-13s %-8s %-4s  %s", icon, item["type"], aff, tag, label)

    log.info(
        "── Operation laydown: %d / %d objects planted ─────────", n_ok, len(all_items)
    )
...
async def plant_opords(client: httpx.AsyncClient, admin_op: SimOp) -> None:
    ...
    n_created = 0
    for idx, (coy, obj_name, village, centre, bearing) in enumerate(
        ARDENNES_PLANS, start=1
    ):
        ...
        body = {
            ...
            "task_organization": (
                "1 PL (ME) — 3 rifle squads, 1 WPN sqd (2x M240B)\n"
            ...
            "command_signal": {
                "command": "CDR with ME during PHASE II. XO at TAC CP at AA.",
                ...
                "callsigns": "BLACK 6 (CDR), RED (1 PL ME), WHITE (2 PL SE), BLUE (3 PL RES), GUNNER 6 (FO).",
        ...
        await api(
            client,
            "POST",
            f"/opord/{opord_id}/snapshots/render",
            token=admin_op.token,
            json={
                "label": f"OBJ {obj_name} AO",
                "bbox": bbox,
                "zoom": 13,
...
async def spawn_opfor(client: httpx.AsyncClient, admin_op: SimOp) -> list[dict]:
    ...
    for _coy, _obj, village, centre, _bearing in ARDENNES_PLANS:
        for i in range(OPFOR_PER_VILLAGE):
            ...
            r2 = await api(
                client, "POST", "/tactical-objects", token=admin_op.token, json=payload
            )
    ...
    log.info(
        "OPFOR: spawned %d units across %d villages.", len(spawned), len(ARDENNES_PLANS)
    )
    return spawned


async def run_opfor_movement(
    client: httpx.AsyncClient, admin_op: SimOp, units: list[dict], speed_mult: float
) -> None:
    ...
    while True:
        for u in units:
            ...
            d_n = step_m * math.cos(ang)
            d_e = step_m * math.sin(ang)
            new_lat, new_lon = jitter(u["lat"], u["lon"], d_n, d_e)
            # Constrain to within OPFOR_RADIUS_M of the village centre.
            if (
                dist_m(u["centre"].lat, u["centre"].lon, new_lat, new_lon)
                > OPFOR_RADIUS_M
            ):
                continue
            # Delete the old marker; re-create at the new spot.
            await api(
                client, "DELETE", f"/tactical-objects/{u['id']}", token=admin_op.token
            )
            payload = {
                "type": "ENEMY",
                "symbol_code": u["sidc"],
...
_NL_AGENT_CATALOGUE = [
    (
        ...
        "CBRN_4",
        "SARIN (GB)",
        "Nerve agent release in city centre — mass casualty event",
    ),
...
async def run_cbrn_netherlands(
    client: httpx.AsyncClient, admin_op: SimOp, speed_mult: float
) -> None:
...
async def main() -> None:
    ...
    log.info(
        "Walk %.1f km/h  Infil %.1f km/h  Update every %gs  Enemy every %gs  "
